...
from guided_diffusion.inception import InceptionV3
...
def get_activations(images, model, batch_size=50, dims=2048, device='cpu',
    ...
    for i in range(0, num_images, batch_size):
        batch = torch.from_numpy(np.transpose(images[i:i + batch_size].astype(np.float32) / 127.5 - 1, [0, 3, 1, 2])).to(device)
        # batch = torch.from_numpy(np.transpose(images[i:i + batch_size].astype(np.float32) / 255.0, [0, 3, 1, 2])).to(device)
...
def compute_statistics_of_path(path, model, batch_size, dims, device, save_stats,
                               num_workers=1):
    if save_stats:
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1):
    ...
    m1, s1 = compute_statistics_of_path(paths[0], model, batch_size,
                                        dims, device, False, num_workers)
    m2, s2 = compute_statistics_of_path(paths[1], model, batch_size,
                                        dims, device, True, num_workers)
